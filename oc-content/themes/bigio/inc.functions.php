...
if (function_exists('osc_admin_menu_appearance')) {
    osc_admin_menu_appearance(__('Header logo', 'bigio'), osc_admin_render_theme_url('oc-content/themes/bigio/admin/header.php'), 'header_modern');
    osc_admin_menu_appearance(__('Theme settings', 'bigio'), osc_admin_render_theme_url('oc-content/themes/bigio/admin/settings.php'), 'settings_modern');
    
} else {

    function modern_admin_menu() {
        echo '<h3><a href="#">' . __('Brasil theme', 'bigio') . '</a></h3>
            <ul>
                <li><a href="' . osc_admin_render_theme_url('oc-content/themes/bigio/admin/header.php') . '">&raquo; ' . __('Header logo', 'bigio') . '</a></li>
                <li><a href="' . osc_admin_render_theme_url('oc-content/themes/bigio/admin/settings.php') . '">&raquo; ' . __('Theme settings', 'bigio') . '</a></li>
                <li><a href="' . osc_admin_render_theme_url('oc-content/themes/bigio/admin/map_settings.php') . '">&raquo; ' . __('Map settings', 'bigio') . '</a></li>
...
function theme_modern_regions_map_admin() {
    $regions = json_decode(osc_get_preference('region_maps', 'bigio'), true);
    switch (Params::getParam('action_specific')) {
        case('edit_region_map'):
            $regions[Params::getParam('target-id')] = Params::getParam('region');
            osc_set_preference('region_maps', json_encode($regions), 'bigio');
            osc_add_flash_ok_message(__('Region saved correctly', 'bigio'), 'admin');
            header('Location: ' . osc_admin_render_theme_url('oc-content/themes/bigio/admin/map_settings.php'));
            exit;
            break;
    }
}
...
function theme_modern_admin_regions_message() {
    $regions = json_decode(osc_get_preference('region_maps', 'bigio'), true);
    if (count($regions) < 0) {
        echo '</div><div class="flashmessage flashmessage-error" style="display:block">' . sprintf(__('Wait! There are unassigned map areas in the map. <a href="%s">Click here</a> to assign regions to the map.', 'bigio'), osc_admin_render_theme_url('oc-content/themes/bigio/admin/map_settings.php')) . '<a class="btn ico btn-mini ico-close">x</a>';
    }
}
